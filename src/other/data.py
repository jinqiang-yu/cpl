...
class Data(object):
    ...
    def binarize(self):
        ...
        for r, (name, feats) in enumerate(zip(self.names[:-1], self.feats[:-1])):
            fgroup = []
            if len(feats) != 2:
                vars_ = sorted([self.fvmap.dir[name, v] for v in feats])
                ...
                var = self.fvmap.dir[name, sorted(feats)[0]]
                vfmap[var] = [var]
                vfmap[-var] = [-var]
        ...
        self.samps_ = [s[:] for s in self.samps]
        # rewriting samples
        for i in range(len(self.samps_)):
            samp, out = self.samps_[i][:-1], self.samps_[i][-1]

            self.samps_[i] = []
            for l in samp:
                self.samps_[i].extend(vfmap[l])

            self.samps_[i].append(out)
    ...
    def get_missing(self):
        ...
        for s in self.samps_:
            missing = []

            if len(s) < self.nof_feats + 1:
                r = i = 0
                while i < len(s) - 1:
                    if r in self.ffmap.dir[self.nm2id[self.fvmap.opp[abs(s[i])][0]]]:
                        i += 1
                    else:
                        missing.append(r)
