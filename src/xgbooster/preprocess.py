...
def discretize_dataset(raw_data_path, files, options):
    ...
    if iscomplete:
        ...
        dataset = np.append(xgb.X_train, np.array([[y] for y in xgb.Y_train]), axis=1)
    ...
    for row in dataset:
        ...
        for i in range(len(row[:-1])):
            ...
            if i in numeric_features:
                ...
                for thredshole in intervals:
                    #name = '{0} < {1}'.format(data_raw.columns[i], thredshole)
                    new_row.append(1 if val < thredshole else 0)

                # for val >= max thredshole
                new_row.append(1 if new_row[-1] == 0 else 0)
            else:
                if iscomplete:
                    new_row.append(val)
                else:
                    new_row.append(xgb.categorical_names[i][int(val)])
        # print('xgb.target_name:')
        # print(xgb.target_name)
        val = row[-1]
        if iscomplete:
            ...
            new_row.append(xgb.target_name[int(val)])
